...
class RangeTree :public SearchContext<T1, T2>
{
private:
	...
	RangeTreeNode* split(Node<T1, T2>* begin, Node<T1, T2>* end)
	{
	...
	{
		RangeTreeNode* new_node;
		int size = end - begin;
		new_node = new RangeTreeNode();
		new_node->left = new_node->right = nullptr;
	...
	RangeTreeNode* CreateNode(Node<T1, T2>* begin, Node<T1, T2>* end)
	{
		RangeTreeNode* new_node;
...
public:
	RangeTree(const Node<T1, T2>* begin, const Node<T1, T2>* end, int in_min_split_size)
	...
	void recursive_free(RangeTreeNode* cur_node)
	{
		if (cur_node == nullptr)
		{
			return;
