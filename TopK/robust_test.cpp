...
int main()
{
	...
	temp_data = generate__data<double>(100000, 5001);
	temp_query = generate_query<double>(100000, 10001);
	LinearSearch<double, uint32_t> ls(&temp_data[0], &temp_data[5000]);
	//CascadeLinear<double, uint32_t> cls(&temp_data[0], &temp_data[50000],2048);
	PrioritySearchTree<double, uint32_t> ps(&temp_data[0], &temp_data[5000], 4);
	...
	for (int i = 0; i < 10000; i++)
	{
		...
		result_1 = ls.TopkSearch(temp_query[i].first, temp_query[i].second, 6);
		...
		result_6 = ps.TopkSearch(temp_query[i].first, temp_query[i].second, 6);
